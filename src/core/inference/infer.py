...
def validate(entry: PreparedData, taco_path: str, waveglow_path: str, denoiser_strength: float, sigma: float, conv: SymbolConverter, n_speakers: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def _infer_core(taco_path: str, waveglow_path: str, conv: SymbolConverter, n_speakers: int, speaker_id: int, sentence_pause_s: float, sigma: float, denoiser_strength: float, sampling_rate: int, symbol_ids_sentences: List[List[int]]) -> Tuple[np.ndarray, List[Tuple[int, Tuple[np.ndarray, np.ndarray, np.ndarray], np.ndarray]]]:
  ...
  for i, symbol_ids in enumerate(tqdm(symbol_ids_sentences)):
    sent_id = i + 1
    _logger.info(f"{sent_id}: {conv.ids_to_text(symbol_ids)} ({len(symbol_ids)})")
