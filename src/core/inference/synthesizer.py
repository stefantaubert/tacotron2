...
class Synthesizer():
  def __init__(self, tacotron_path: str, waveglow_path: str, n_symbols: int, n_speakers: int, custom_taco_hparams: str = "", custom_wg_hparams: str = "", logger: logging.Logger = logging.getLogger()):
    super().__init__()
    self._logger = logger
    self._taco_synt = TacoSynthesizer(checkpoint_path=tacotron_path, n_speakers=n_speakers, n_symbols=n_symbols, custom_hparams=custom_taco_hparams, logger=logger)
    self._wg_synt = WGSynthesizer(checkpoint_path=waveglow_path, custom_hparams=custom_wg_hparams, logger=logger)

  def infer(self, symbol_ids: List[int], speaker_id: int, sigma: float, denoiser_strength: float) -> Tuple[Tuple, list]:
