...
def process(data: TextDataList, ignore_tones: bool, ignore_arcs: bool) -> Tuple[TextDataList, SymbolConverter, OrderedDict[str, int]]:
  result: TextDataList = []
  entry_symbols = []
  ...
  for values in tqdm(data):
    ...
    result.append(TextData(values.entry_id, ipa, "", Language.IPA))
