...
class UnitTests(unittest.TestCase):
  ...
  def test_get_similarities_is_sorted(self):
    symbols = SymbolConverter.init_from_symbols({"a"})
    emb = np.zeros(shape=(3,3))
    emb[symbols.symbol_to_id("_")] = [0.5, 1.0, 0]
    emb[symbols.symbol_to_id("~")] = [1.0, 0.6, 0]
    emb[symbols.symbol_to_id("a")] = [1.0, 0.5, 0]

    sims = get_similarities(emb)

    self.assertEqual(3, len(sims))
    self.assertEqual(symbols.symbol_to_id("~"), sims[symbols.symbol_to_id("_")][0][0])
    self.assertEqual(symbols.symbol_to_id("a"), sims[symbols.symbol_to_id("_")][1][0])
    self.assertEqual(symbols.symbol_to_id("~"), sims[symbols.symbol_to_id("a")][0][0])
    self.assertEqual(symbols.symbol_to_id("_"), sims[symbols.symbol_to_id("a")][1][0])
    self.assertEqual(symbols.symbol_to_id("a"), sims[symbols.symbol_to_id("~")][0][0])
    self.assertEqual(symbols.symbol_to_id("_"), sims[symbols.symbol_to_id("~")][1][0])

  def test_sims_to_csv(self):
    emb = torch.ones(size=(2,3))
    torch.nn.init.zeros_(emb[0])
    sims = get_similarities(emb)
    symbols = SymbolConverter.init_from_symbols({})
